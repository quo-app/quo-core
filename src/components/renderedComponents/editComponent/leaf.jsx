...
const makeLeaf = (WrappedComponent, options) => {
  return class extends React.PureComponent {
    ...
    onMouseDown = e => {
      if(!this.state.childrenInFocus){
        this.selectComponent();
        e.stopPropagation();
      }
      else if (this.state.childrenInFocus){
    ...
    makeChildrenSelectable = () => {
      this.setState({ childrenInFocus: true });
      const { dispatch } = this.props;
    ...
    makeChildrenUnselectable = () => {
      this.setState({ childrenInFocus: false });
    }
    ...
    onMouseMove = e => {
      ...
      const componentDOM = document.getElementById(`component-${this.props.component.id}`);
    ...
    render = () => {
      ...
      return(
        <div {...wrapperProps}>
          <WrappedComponent {...this.props} wrapper={LeafComponent} type={'edit'}/>
        </div>
      )
    }
  }
...
const LeafComponent = connect(mapStateToProps)(makeLeaf(ComponentRender))
